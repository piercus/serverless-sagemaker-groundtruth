...
module.exports = function ({workerId, prelambdaOutput, manifestRow, puppeteerMod, url}) {
	...
	return puppeteer.launch()
		.then(browser => {
			return browser.newPage()
				...
				.then(jsonString => {
					return browser.close().then(() => {
						...
						return parsed[0].annotatedResult;
					});
				});
		}).then(annotatedResult => {
			// This is only working for custom task on aws Sagemaker
			// For prebuild task, amazon is adding a key
			// For example for boundingBox, we should do something like {workerId, annotationData: {content : JSON.stringify({boundingBox: annotatedResult})}}
			...
			return {workerId, annotationData: {content: JSON.stringify(annotatedResult)}};
		});
};
