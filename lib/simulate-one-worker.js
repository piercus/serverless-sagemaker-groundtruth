					...
					return page.goto(url, {waitUntil: 'networkidle0'})
						...
						.then(() => 
							page.waitForFunction((selector) => {
								const btn = querySelectorShadowDom.querySelectorDeep(selector);
								return btn;
							}, {}, submitSelector).catch(err => {
								throw(new Error(`The page submit button should match selector ${submitSelector}`))
							}).then(a => a.asElement())
						)
						.then(btn => {
						...
						.then(() => {
							return page.waitForFunction(() => {
								...
								return page.screenshot({path: debuggingScreenshot}).then(() => {
									throw(new Error(`Not able to get the code from the page, the json output should appear in a $("table code") element, (screenshot saved in ${debuggingScreenshot})`))
								})
							})
							.then(a => a.jsonValue());
						});
